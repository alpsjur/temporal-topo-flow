...
if params["name"] == "slope-052":   
    datamean = load_dataset(params["filepath"], "slope-050")
elif params["name"] == "slope-053":
    datamean = load_dataset(params["filepath"], "slope-051")
else:
    print("you are running the wrong script for this configuration, friend")

Tn = int(params["T"]/params["outputtime"])

um = np.mean(datamean['u'].values[-Tn:], axis=0)  # Shape: (time, y, x)
vm = np.mean(datamean['v'].values[-Tn:], axis=0)  # Shape: (time, y, x)

# Compute time-averaged velocity field
u_res = u - um 
v_res = v - vm

u_mean = np.mean(u_res[-Tn:], axis=0)
v_mean = np.mean(v_res[-Tn:], axis=0)
