...
uc_timeseries = deepcopy(h_timeseries)  # u-component interpolated on the h grid
vc_timeseries = deepcopy(h_timeseries)  # v-component interpolated on the h grid
s_timeseries = deepcopy(h_timeseries)   # Speed interpolated on the h grid
ω_timeseries = deepcopy(h_timeseries)   # Vorticity interpolated on the h grid
...
for i in 1:length(times)
    ...
    ω_timeseries[i] .= @at (Center, Center, Center) ∂x(vᵢ) - ∂y(uᵢ)
end
...
hₙ = @lift interior(h_timeseries[$n], :, :)  # Height field
sₙ = @lift interior(s_timeseries[$n], :, :)  # Speed field
ωₙ = @lift interior(ω_timeseries[$n], :, :)  # Vorticity field
...
ax1 = Axis(fig[2, 1]; title = "depth [m]")            # Axis for depth plot
